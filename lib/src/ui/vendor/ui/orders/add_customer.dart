...
class _AddCustomerState extends State<AddCustomer> {
  ...
  Widget buildListView(
    ...
    if (snapshot.data.countries.indexWhere(
            (country) => country.value == widget.order.billing.country) !=
        -1) {
    ...
    if (regions != null && regions.isNotEmpty) {
      widget.order.billing.state =
          regions.any((z) => z.value == widget.order.billing.state)
              ? widget.order.billing.state
              : regions.first.value;
    }
                    ...
                    : PrimaryColorOverride(
                        child: TextFormField(
                          ...
                          onSaved: (val) => setState(() => widget
                              .vendorBloc.formData['billing_state'] = val),
                        ),
                      ),
                SizedBox(
                  height: 24.0,
                    ...
                    ? Column(
                        children: <Widget>[
                          ...
                          DropdownButton<String>(
                            ...
                            onChanged: (String newValue) {
                              setState(() {
                                widget.order.shipping.state = newValue;
                              });
                            },
                            items:
                                regions.map<DropdownMenuItem<String>>((value) {
