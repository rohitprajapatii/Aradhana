...
class VendorReviewsList extends StatefulWidget {
  ...
  const VendorReviewsList({Key key, this.vendorDetailModel}) : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    return ScopedModelDescendant<VendorDetailStateModel>(
      builder: (context, child, model) {
        return model.reviews != null && model.reviews.length > 0
            ? CustomScrollView(
                controller: _reviewScrollController,
                slivers: buildList(model.reviews, context))
            : CustomScrollView(controller: _reviewScrollController, slivers: [
                writeReviewTile(),
                SliverToBoxAdapter(
