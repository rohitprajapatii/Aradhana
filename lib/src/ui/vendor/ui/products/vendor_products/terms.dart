...
class AttributeOptionsPage extends StatefulWidget {
  ...
  AttributeOptionsPage(
      {Key key, this.productAttribute, this.product, this.attributeBloc})
      : super(key: key);
  @override
...
class _AttributeOptionsPageState extends State<AttributeOptionsPage> {
  ...
  buildBody(BuildContext ctxt, AttributeTerms attributesTerm) {
    ...
    if (widget.product.attributes
        .any((item) => item.id == widget.productAttribute.id)) {
      if (widget.product.attributes
          .singleWhere((item) => item.id == widget.productAttribute.id)
          .options
          .contains(attributesTerm.name)) {
        contains = true;
      }
    }
    return ListTile(
      ...
      trailing: Checkbox(
        value: contains,
        onChanged: (bool value) => _onAttributesTermsTap(attributesTerm),
      ),
    );
  }
