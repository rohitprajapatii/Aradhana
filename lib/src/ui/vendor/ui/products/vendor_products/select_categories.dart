...
class CategoryList extends StatefulWidget {
  ...
  final VendorProduct product;
  CategoryList({Key key, this.categories, this.product}) : super(key: key);
  @override
...
class _CategoryListState extends State<CategoryList> {
  ...
  void onCategoryClick(Category category) {
    ...
    } else if (widget.product.categories
        .any((value) => value.id == category.id)) {
      setState(() {
        widget.product.categories
            .removeWhere((value) => value.id == category.id);
      });
    }
  }
  ...
  Widget _buildTiles(Category category) {
    ...
    if (subCategories.isEmpty) {
      ...
      return ExpansionTile2(
        ...
        leading: Checkbox(
          value:
              widget.product.categories.any((item) => item.id == category.id),
          onChanged: (bool value) {
            if (value) {
              ProductCategory productCategory = new ProductCategory();
              productCategory.id = category.id;
              productCategory.name = category.name;
              if (!widget.product.categories
                  .any((value) => value.id == category.id)) {
                setState(() {
                  widget.product.categories.add(productCategory);
                });
