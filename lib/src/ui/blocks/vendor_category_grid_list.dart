...
class VendorCategoryGridList extends StatefulWidget {
  ...
  VendorCategoryGridList(
      {Key key, this.block, this.categories, this.onCategoryClick})
      : super(key: key);
  @override
...
class _VendorCategoryGridListState extends State<VendorCategoryGridList> {
  ...
  Widget build(BuildContext context) {
    ...
    final childAspectRatio = (containerWidth / (crossAxisCount * 16)) /
        ((containerWidth / (crossAxisCount * 16)) * 1.25);
    ...
    return SliverPadding(
      ...
      sliver: SliverGrid(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: isDisplayDesktop(context)
                ? crossAxisCount
                : int.parse(widget.block.layoutGridCol.toString()),
            childAspectRatio: isDisplayDesktop(context)
                ? childAspectRatio
                    ...
                    child: Card(
                      ...
                      child: InkWell(
                        ...
                        onTap: () => widget.onCategoryClick(
                            widget.categories[index], widget.categories),
                        child: Column(
                          children: <Widget>[
                            AspectRatio(
                              aspectRatio: 18.0 / 14.0,
                              child: widget.categories[index].image != null
                                  ? CachedNetworkImage(
...
class VendorCategoryStadiumGridList extends StatefulWidget {
  ...
  final List<Category> categories;
  final Function onCategoryClick;
  VendorCategoryStadiumGridList(
      {Key key, this.block, this.categories, this.onCategoryClick})
      : super(key: key);
  @override
  _VendorCategoryStadiumGridListState createState() =>
      _VendorCategoryStadiumGridListState();
}

class _VendorCategoryStadiumGridListState
    extends State<VendorCategoryStadiumGridList> {
  int count;
  ...
  Widget build(BuildContext context) {
    ...
    final childAspectRatio = (containerWidth / (crossAxisCount * 16)) /
        ((containerWidth / (crossAxisCount * 16)) * 1.35);
  ...
  _viewAll() {
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                VendorAllCategories(categories: widget.categories)));
  }
}
