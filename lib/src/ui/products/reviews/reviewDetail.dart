...
class ReviewsDetail extends StatefulWidget {
  ...
  const ReviewsDetail({Key key, this.productDetailBloc, this.product})
      : super(key: key);
...
class _ReviewsDetailState extends State<ReviewsDetail> {
  ...
  Column buildReviewSummary(
      AsyncSnapshot<List<ReviewModel>> snapshot, BuildContext context) {
    var fiveStars = snapshot.data
        .where((element) =>
            double.parse(element.rating) > 4 &&
            double.parse(element.rating) <= 5)
        .length;
    var fourStars = snapshot.data
        .where((element) =>
            double.parse(element.rating) > 3 &&
            double.parse(element.rating) <= 4)
        .length;
    var threeStars = snapshot.data
        .where((element) =>
            double.parse(element.rating) > 2 &&
            double.parse(element.rating) <= 3)
        .length;
    var twoStars = snapshot.data
        .where((element) =>
            double.parse(element.rating) > 1 &&
            double.parse(element.rating) <= 2)
        .length;
    var singleStars = snapshot.data
        .where((element) =>
            double.parse(element.rating) > 0 &&
            double.parse(element.rating) <= 1)
        .length;
