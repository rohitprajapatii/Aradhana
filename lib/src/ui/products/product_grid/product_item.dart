  ...
  Widget build(BuildContext context) {
    ...
    final containerWidth = isDesktop
        ? MediaQuery.of(context).size.width -
            desktopCategoryMenuPageWidth(context: context)
        : MediaQuery.of(context).size.width;
...
class _ProductItemState extends State<ProductItem> {
  ...
  Widget build(BuildContext context) {
    ...
    if ((widget.product.salePrice != null && widget.product.salePrice != 0)) {
      percentOff = (((widget.product.regularPrice -
  ...
  decreaseQty() async {
    if (appStateModel.shoppingCart?.cartContents != null) {
      if (appStateModel.shoppingCart.cartContents
        ...
        final cartContent = appStateModel.shoppingCart.cartContents.singleWhere(
  ...
  increaseQty() async {
    if (appStateModel.shoppingCart?.cartContents != null) {
      if (appStateModel.shoppingCart.cartContents
        ...
        bool status = await appStateModel.increaseQty(
            cartContent.key, cartContent.quantity);
        setState(() {
          isLoading = false;
        });
      }
  ...
  getQty(AppStateModel model) {
    if (model.shoppingCart.cartContents
