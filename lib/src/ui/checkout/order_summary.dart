...
class _OrderSummaryState extends State<OrderSummary> {
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData && snapshot.data.id != null) {
                final NumberFormat formatter = NumberFormat.currency(
                    decimalDigits: snapshot.data.decimals,
                    name: snapshot.data.currency);
  ...
  Widget buildOrderDetails(
      Order order, BuildContext context, NumberFormat formatter) {
    return SliverList(
  ...
  void onSuccessMessage(Order order) {
    showDialog(
        ...
        child: AlertDialog(
          title: new Text(
              widget.appStateModel.blocks.localeText.youOrderHaveBeenReceived),
          content: new Text(
              widget.appStateModel.blocks.localeText.thankYouForShoppingWithUs +
                  '!' +
                  widget.appStateModel.blocks.localeText.thankYouOrderIdIs +
                  ':' +
                  ' #${order.id.toString()}. ' +
          ...
          actions: <Widget>[
            RaisedButton(
                elevation: 0,
                child: Text(
                    widget.appStateModel.blocks.localeText.localeTextContinue),
                onPressed: () {
                  Navigator.of(context).pop();
                  Navigator.popUntil(
