...
class ProductDetailBloc {
  ...
  ValueStream<ReleatedProductsModel> get relatedProducts =>
      _relatedProductFetcher.stream;
  ValueStream<List<ReviewModel>> get allReviews => _reviewsFetcher.stream;

  void getProductsDetails(int id) async {
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=mstore_flutter-product_details',
        {'product_id': id.toString()});
    if (response.statusCode == 200) {
      ReleatedProductsModel productsList =
          releatedProductsFromJson(response.body);
  ...
  void getReviews(int id) async {
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=mstore_flutter-product_reviews',
        {'product_id': id.toString()});
    if (response.statusCode == 200) {
  ...
  getProduct(int id) async {
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=mstore_flutter-product',
        {'product_id': id.toString()});
    if (response.statusCode == 200) {
