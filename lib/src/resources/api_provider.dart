...
class ApiProvider {
  ...
  Future<http.Response> fetchBlocks() async {
    ...
    cookies = prefs.getString('cookies') != null
        ? json.decode(prefs.getString('cookies'))
        : {};
    ...
    final response = await http.post(
      Uri.parse(
          config.url + '/wp-admin/admin-ajax.php?action=mstore_flutter-keys'),
      headers: headers,
      body: filter,
    );
    return response;
  ...
  Future<List<Product>> fetchProductList(Map data) async {
    ...
    final response = await http.post(
      Uri.parse(config.url +
          '/wp-admin/admin-ajax.php?action=mstore_flutter-products'),
      headers: headers,
      body: data,
    );
  ...
  String generateWebViewCookieHeader() {
    String cookie = "";
    for (var key in cookies.keys) {
      if (key.contains('woocommerce') || key.contains('wordpress')) {
        if (cookie.length > 0) cookie += "; ";
        cookie += key + "=" + cookies[key];
      }
    }
